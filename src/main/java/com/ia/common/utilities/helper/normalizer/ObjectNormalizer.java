...
public interface ObjectNormalizer<T> extends Normalizer<T, Map<String, Object>> {
    ...
    default BiFunction<T, T, Map<String, ObjectDiffDetails>> differenceExtractor() {
        return (actual, expected) -> this.commonDifferenceExtractor(List.of()).apply(actual, expected);
    }
    ...
    default BiFunction<T,T, Map<String, ObjectDiffDetails>> differenceExtractor(List<String> excludedKeys) {
        return (left, right) -> this.commonDifferenceExtractor(excludedKeys).apply(left, right);

    }

    private BiFunction<T, T, Map<String, ObjectDiffDetails>> commonDifferenceExtractor(List<String> excludedKeys) {
        return (actual, expected) -> {
            ...
            return getMaxKeys(actualMap, expectedMap, excludedKeys).stream()
                    .map(k -> {
